...
public:
    ThreadIterator(const vector<Thread*> *collection);
    void First();
    void Next();
    Thread* CurrentItem() const;
    bool IsDone()const;
private:
    const vector<Thread*> *_collection;
    int _current;
...
ThreadIterator::ThreadIterator(const vector<Thread*> *collection) :
_collection(collection), _current(0) {
}
...
bool ThreadIterator::IsDone() const {
    return _current >= _collection->size();
}
